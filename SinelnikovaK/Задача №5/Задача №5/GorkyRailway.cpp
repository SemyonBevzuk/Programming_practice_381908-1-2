...
int GorkyRailway::carReservate(const int train, const int passengers)
{
	...
	if (train == 3)
	{
		...
		if (tmp != 0)
		{
			...
			{
				cout << "Выберите тип вагона из представленных: ";
				cin >> carriage;
			}
			if (carriage == 1)
				return sv;
...
void GorkyRailway::typeCar(const int train, const int passengers, const int carriage, int** seats)
{
	...
	else if (train == 3)
	{
		...
		{
			cout << "Нижние места: " << firmReserved[carriage][0] << "\nВерхние места: " << firmReserved[carriage][1] << '\n';
			int down(-1), up;
	...
	else
	{
		if (carriage < 4)
		{
			cout << "Нижние места: " << fastCoupe[carriage][0] << "\nВерхние места: " << fastCoupe[carriage][1] << '\n';
			int down(-1), up;
		...
		{
			cout << "Нижние места: " << fastReserved[carriage][0] << "\nВерхние места: " << fastReserved[carriage][1] << '\n';
			int down(-1), up;
